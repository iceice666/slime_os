...
BUILD_GENERATION = load_script("build_generation", "build/build-generation.py")
BUILD_TRANSFER = load_script("build_transfer", "build/build-transfer.py")
...
def main() -> None:
    ...
    run(
        [str(ROOT / "scripts" / "build" / "build-generation.py"), str(KERNEL), str(generated)],
        environment,
    )
    ...
    run(
        [str(ROOT / "scripts" / "build" / "build-generation.py"), str(KERNEL), str(source_generated)],
        source_environment,
    )
