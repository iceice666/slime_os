...
KERNEL = ROOT / "kernel"
BUILD = ROOT / "target" / "x86_64-unknown-none" / "release"
WORK = Path("/tmp/slime-os-generation-cmd")
TRACE = load_script("check_bootstate_trace", "check/check-bootstate-trace.py")
...
def build_fixture(scenario: str) -> Path:
    ...
    subprocess.run(
        [str(ROOT / "scripts" / "build" / "build-generation.py"), str(BUILD / "slime_os-kernel"), str(generated)],
        cwd=ROOT,
        env=environment,
        check=True,
    )
