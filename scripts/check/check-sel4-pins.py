...
def check_toolchain_and_targets(pins: dict[str, object]) -> None:
    ...
    # Only repo-owned specifications are pinned here. The AArch64 and RISC-V
    # child specifications live in the pinned rust-sel4 checkout and are
    # resolved from it by name, so they carry no `sel4/pins.toml` entry; the
    # x86-64 pair is repo-owned and both halves are pinned.
    target_specs = (
...
def check_profile(pins: dict[str, object]) -> None:
    ...
    # marker gate depends on. Six entries are load-bearing beyond that:
    #
    ...
    #  * `KernelMultibootGFXMode "linear"` is what makes the kernel emit the
    #    Multiboot2 framebuffer request tag, without which a bootloader leaves
    #    the machine in text mode and exports no framebuffer record. It is the
    #    only readiness channel a serial-less Framework has. Width and height
    #    are 0 — no preference — because the pinned q35 target and the physical
    #    panel share no mode list; depth is a hint the root re-validates.
    required_pc99 = {
        "KernelPlatform": "pc99",
        ...
        "KernelMultibootGFXMode": "linear",
        "KernelMultibootGFXWidth": "0",
        "KernelMultibootGFXHeight": "0",
        "KernelMultibootGFXDepth": "32",
    }
...
def check_prefix(pins: dict[str, object], platform: str) -> None:
    ...
    # Platforms without a device tree install no `platform_gen.yaml`, so the
    # window check has no file to read and no pinned window to compare; it
    # returns early for them, and `files` carries no such key to pass.
    if "platform_info_sha256" in files:
