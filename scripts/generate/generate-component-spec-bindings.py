...
"""Render `contracts/component-spec/v1`'s host constants.
...
CONTRACT_GENERATOR = ROOT / "contracts" / "component-spec" / "v1" / "gen_python.zt"
PYTHON_OUTPUT = ROOT / "scripts" / "lib" / "component_spec_contract.py"
LOCK_PATH = Path(tempfile.gettempdir()) / "slime-component-spec-bindings.lock"


def render() -> str:
    with tempfile.TemporaryDirectory(prefix="slime-component-spec-bindings-") as temporary:
        staging = Path(temporary)
        environment = os.environ.copy()
        environment["ZUTAI_STDLIB_ROOT"] = str(STDLIB)
        environment["SLIME_COMPONENT_SPEC_BINDINGS_ROOT"] = str(staging)
        ...
        if process.returncode != 0:
            raise SystemExit(
                "component-spec binding generator failed: "
                f"{(process.stderr or process.stdout).strip()}"
            )
        output = staging / "component_spec_contract.py"
        if not output.is_file():
            raise SystemExit("component-spec binding generator wrote no output")
        ...
        if arguments.check:
            ...
            if existing != rendered:
                raise SystemExit(
                    ...
                    "run python3 scripts/generate/generate-component-spec-bindings.py"
                )
