...
RUST_OUTPUT = ROOT / "components" / "proto" / "src" / "store.rs"
...
def render() -> dict[Path, str]:
    with tempfile.TemporaryDirectory(prefix="slime-store-bindings-") as temporary:
        ...
        staged_rust = staging / "components" / "proto" / "src" / "store.rs"
        ...
        if not staged_rust.exists():
            raise SystemExit("store generator did not write the binding surface")
