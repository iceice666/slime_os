...
def render() -> str:
    with tempfile.TemporaryDirectory(prefix="slime-system-image-closure-bindings-") as temporary:
        staging = Path(temporary)
        environment = os.environ.copy()
        environment["ZUTAI_STDLIB_ROOT"] = str(STDLIB)
        environment["SLIME_SYSTEM_IMAGE_CLOSURE_BINDINGS_ROOT"] = str(staging)
        ...
        if process.returncode != 0:
            raise SystemExit(
                "system-image-closure binding generator failed: "
                f"{(process.stderr or process.stdout).strip()}"
            )
        output = staging / "system_image_closure_contract.py"
        if not output.is_file():
            raise SystemExit("system-image-closure binding generator wrote no output")
        ...
        if arguments.check:
            ...
            if existing != rendered:
                raise SystemExit(
                    ...
                    "run python3 scripts/generate/generate-system-image-closure-bindings.py"
                )
