...
OUTPUT = ROOT / "components" / "proto" / "src" / "fabric_visibility.rs"
...
def render() -> str:
    with tempfile.TemporaryDirectory(prefix="slime-fabric-visibility-bindings-") as temporary:
        ...
        staged = staging / "components" / "proto" / "src" / "fabric_visibility.rs"
        ...
        if not staged.exists():
            raise SystemExit("fabric-visibility generator did not write bindings")
...
def main() -> None:
    ...
    if arguments.check:
        if not OUTPUT.exists() or OUTPUT.read_text(encoding="utf-8") != generated:
            raise SystemExit(
                "generated fabric-visibility bindings are stale; run `just fabric_visibility_gen`"
            )
