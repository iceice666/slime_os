...
GENERATOR = ROOT / "contracts" / "pwm-servo" / "v1" / "schema.zt"
RUST_OUTPUT = ROOT / "components" / "proto" / "src" / "pwm_servo.rs"
C_OUTPUT = ROOT / "components" / "runtime" / "include" / "slime" / "pwm_servo.h"
INVALID_SCHEMA = "INVALID_PWM_SERVO_SCHEMA"


def render() -> tuple[str, str]:
    with tempfile.TemporaryDirectory(prefix="slime-pwm-servo-bindings-") as temporary:
        ...
        environment["SLIME_PWM_SERVO_BINDINGS_ROOT"] = str(staging)
        ...
        if not staged_rust.exists() or not staged_c.exists():
            raise SystemExit("pwm-servo generator did not write both bindings")
        ...
        if INVALID_SCHEMA in generated_rust or INVALID_SCHEMA in generated_c:
            raise SystemExit("pwm-servo schema reflection/layout validation failed")
    ...
    if arguments.check:
        for output, generated in outputs:
            if not output.exists() or output.read_text(encoding="utf-8") != generated:
                raise SystemExit("generated pwm-servo bindings are stale; run `just pwm_servo_gen`")
        print("pwm-servo protocol bindings are current")
        return
    for output, generated in outputs:
