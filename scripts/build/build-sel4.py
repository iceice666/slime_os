...
def build_sel4_generation() -> Path:
    """Build the `aarch64-sel4-qemu-virt` generation the root task launches.
    ...
    regenerated by hand.
    """
    output = BUILD_ROOT / "sel4-generation"
...
def build_application(
    pins: dict[str, object], *, component_graph: bool = False
) -> tuple[Path, Path]:
    ...
    # P5.2's graph is opt-in so that one script serves both gates from one
    # source: with `--component-graph` the root task embeds the
    # `aarch64-sel4-qemu-virt` generation and launches its five components;
    # without it the retained x86 generation is embedded and the P5.1 native
    # fixture path runs, unchanged. The root task chooses between them by what
    # the generation carries, not by a flag it was built with.
    if component_graph:
        root_environment["SLIME_GENERATION"] = str(build_sel4_generation().resolve())
    # Separate target directories: the two images embed different generations,
    # so sharing one would make each build invalidate the other's artifact and
    # whichever gate ran second would boot a rebuilt image.
    root_target_dir = CARGO_BUILD / ("root-graph" if component_graph else "root")
    cargo_build(
...
def write_manifest(
    ...
    *,
    ...
    suffix = "-graph" if component_graph else ""
    ...
    manifest = {
        ...
        # Which startup path this image takes, so a gate cannot boot the other
...
def main() -> None:
    ...
    child_elf, root_elf = build_application(pins, component_graph=arguments.component_graph)
    loader, payload_tool = build_loader(pins)
    image = GRAPH_IMAGE if arguments.component_graph else IMAGE
    manifest_path = GRAPH_MANIFEST if arguments.component_graph else MANIFEST
    ...
    write_manifest(
        ...
        component_graph=arguments.component_graph,
    )
    print(
