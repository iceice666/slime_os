...
def build_application(pins: dict[str, object]) -> tuple[Path, Path]:
    ...
    root_target_dir = CARGO_BUILD / "root"
    cargo_build(
...
def package_image(payload_tool: Path, loader: Path, root_elf: Path) -> None:
    run(
        [
            str(payload_tool),
            "--sel4-prefix",
...
def write_manifest(
    ...
    *,
    ...
    stable_child = copy_artifact(child_elf, "slime-root-child.elf")
    stable_root = copy_artifact(root_elf, "slime-root.elf")
    ...
    try:
        ...
        fail(f"cannot write {MANIFEST.relative_to(ROOT)}: {error}")
...
def main() -> None:
    ...
    child_elf, root_elf = build_application(pins)
    loader, payload_tool = build_loader(pins)
    package_image(payload_tool, loader, root_elf)
    write_manifest(
        pins,
        child_elf=child_elf,
        root_elf=root_elf,
