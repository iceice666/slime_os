...
def host_path_needles(source: Path = ROOT) -> tuple[bytes, ...]:
    """Absolute host paths an exported byte may never contain.

    Derived from this process's own environment rather than pattern-matched: a
    ...
    return (
        ...
        tempfile.gettempdir().encode("utf-8"),
        b"/nix/store/",
    )
    ...
    if not prefix.is_dir():
        ...
        with tarfile.open(archive, mode="r:") as opened:
            for info in opened.getmembers():
                if not info.isfile():
                    fail("prefix archive member " + repr(info.name) + " is not a regular file")
...
REV = re.compile(r'(rev\\s*=\\s*")([0-9a-f]{40})(")')
GIT = re.compile(r'(git\\s*=\\s*")([^"]+)(")')
...
def main() -> None:
    ...
    try:
        ...
        if REV.search(text) is None:
            fail("consumer manifest declares no full-commit SDK pin to update")
        text = REV.sub(lambda match: match.group(1) + arguments.sdk_commit + match.group(3), text)
        text = GIT.sub(lambda match: match.group(1) + arguments.sdk_url + match.group(3), text)
        (staging / "Cargo.toml").write_text(text, encoding="utf-8")
        ...
        run([sys.executable, str(BUILD), "--profile", arguments.profile, "--verify-only"]
            + (["--cache", arguments.cache] if arguments.cache else []),
            staging, "release verification")
        run(["cargo", "update", "--manifest-path", str(staging / "Cargo.toml")],
            staging, "dependency resolution")
        run(build, staging, "component rebuild")
    ...
    for entry in sorted(consumer.iterdir()):
    ...
    for entry in sorted(staging.iterdir()):
        destination = consumer / entry.name
        if entry.name == ".git":
            continue
        if entry.is_dir():
...
def _write_record(sdk: Path, record: dict, contract: ModuleType) -> tuple[bytes, bytes]:
    ...
    (sdk / contract.RECORD_FILE_NAME).write_text(zti(record) + "\n", encoding="utf-8")
...
def load_record(sdk: Path, contract: ModuleType = default_contract) -> dict:
    ...
    for path in (normalized_path, identity_path, sdk / contract.RECORD_FILE_NAME):
        if not path.is_file():
...
def matrix_row(
    ...
    *,
    ...
    if classification not in contract.CLASSIFICATIONS:
        _fail(f"unknown classification {classification!r}")
    if not evidence:
    ...
    if _COMMIT.fullmatch(sdk_commit) is None:
        _fail(f"matrix row names a non-immutable SDK commit: {sdk_commit!r}")
    if _COMMIT.fullmatch(product_commit) is None:
        _fail(f"matrix row names a non-immutable product commit: {product_commit!r}")
    if profile not in PROFILE_PLATFORMS:
        _fail(f"matrix row names an unknown target profile: {profile!r}")
    return {
...
def matrix(rows: list[dict], contract: ModuleType = default_contract) -> dict:
    ...
    seen = {(row["sdkCommit"], row["productCommit"], row["profile"]) for row in rows}
    if len(seen) != len(rows):
        _fail("compatibility matrix declares one pairing twice")
    return {"formatVersion": contract.FORMAT_VERSION, "rows": rows}


def supported(table: dict, *, sdk_commit: str, product_commit: str, profile: str) -> bool:
