...
# One bounded entry with no timeout and no interactive path.
#
# The console is both channels, not one. `serial` is the evidence channel an
# emulated gate reads, but a physical Framework has no 16550, and naming
# serial as the *only* terminal sent GRUB's own diagnostics to a device that
# does not exist there. `console` is the firmware's own text output, which the
# first physical boot attempt proved works on that machine — a GRUB message
# was legible on the panel even while the configuration named serial alone.
# Listing both means a bootloader-stage failure is readable on the machine it
# happens on, without adding a font file the pinned four-file tree does not
# carry.
#
# The readiness record itself needs a linear framebuffer: `insmod all_video`
# plus `gfxpayload=keep` is what makes the bootloader negotiate a mode through
# EFI GOP and hand the kernel a Multiboot2 framebuffer tag. `gfxmode` stays
# `auto` rather than a pinned geometry because the pinned q35 target and the
# Framework panel do not share a mode list. Both sides are required, observed
# rather than assumed: with the modules absent the kernel reports no
# framebuffer at all, and with the kernel's own header tag absent GRUB
# supplies none.
GRUB_CONFIG = """\
set timeout=0
set default=0
...
def qemu_command(
    *,
    ...
    """The pinned q35/OVMF command line for one pc99 boot source.

    P6.2 boots the assembled tree through QEMU's synthetic FAT adapter. P6.5
    passes ``raw_image`` so QEMU reads the exact GPT/FAT32 bytes approved for
    the Framework USB boot. Machine, CPU, firmware, and writable OVMF-variable
    handling remain identical between the two paths.
    """
    from harness import profile_integer, profile_text  # noqa: PLC0415
    ...
    if raw_image is not None and not raw_image.is_file():
        fail(f"raw boot image is absent: {raw_image}")
    ...
    for feature in features:
        cpu += f",+{feature}"
    boot_drive = (
        f"format=raw,snapshot=on,file={raw_image}"
        if raw_image is not None
        else f"format=raw,file=fat:rw:{tree}"
    )
    return [
        qemu,
        "-machine",
