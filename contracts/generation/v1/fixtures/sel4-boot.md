...
## The control channels are minted, so the layout has 21 rows and not 53
...
component's layout slots. `slime-root` numbers a launched component's declared
ends from its own cursor, which resumes above the factory grants staging
installs — so a declared control reaches the fabric at a slot no
`FABRIC_FIRST_CONTROL_SLOT + index` describes.

Observed directly: the first boot of this fixture declared the controls, the
fabric received its ends at cursor positions, and every worker spawn failed.

So `init.rs::drive_boot_plane` mints the sixteen pairs and hands each participant
its own half at spawn. The grants stay in the manifest because `_control_sources`
derives the identity tables from those names — they name, the minted endpoints
authorize — and `SEL4_BOOT_LAYOUT` numbers only the twenty-one things the
*generation* places: two factories, three service and worker executables, and
sixteen participant executables.
