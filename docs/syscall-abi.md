...
| 34 | `CAPABILITY IMPORT` | `MR0=0`, `MR1=0` or `endpoint_slot + 1` | The slot the claimed capability landed in. With `MR1=0` the oldest finalized export addressed to the caller is claimed from any sender; with an endpoint slot named, only an export from that endpoint's peer instance is claimed, which is how a service with several clients keeps one client's descriptor from taking up another's loan. A slot that binds no instance-to-instance grant, or whose peer is not live, claims nothing. |
...
| 70 | `CLOCK RATE READ` | none | Primary is the platform monotonic counter's rate in ticks per second, as the root itself reads it at boot. Requires only `monotonicRead`, the same bit as `CLOCK MONOTONIC READ`: a holder that may read the counter may convert its ticks. The value is constant for the life of the boot and never comes from a component-side table. |
| 71 | `CAPABILITY NETWORK INTERFACE READ` | `MR0=cursor`, `MR1=0`, `MR2=transfer descriptor` | Primary is the number of `network-interface/v1` entries written to the caller's transfer window. The caller is badge-derived and must be the generation's declared `network-service`; no operand names a holder or interface. The root copies authenticated bytes only; which row is the service's own, and what it configures, is decided in userspace. |
