...
| 62 | `IO RESOURCE IRQ WAIT ACK` | `MR0=interrupt_source_slot`, `MR1=epoch`, `MR2=prior_sequence` | Primary is the next interrupt sequence. Sequence zero binds and waits without acknowledging; a nonzero sequence acknowledges exactly that pending event before waiting. Spoofed, duplicate, and stale acknowledgements are refused. |
