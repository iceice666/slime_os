...
| 38 | `CAPABILITY GRAPH READ` | `MR0=cursor`, `MR1=0`, `MR2=transfer descriptor` selecting the caller's reply window | The declared participant rows of this generation's fabric graph, from `cursor` onward, written into the caller's window and answered as the count returned plus a descriptor; the caller resumes from `cursor + count` until the count is short. Answered **only** to the instance the graph names as its fabric component: `FabricGraph` carries a `fabricComponentIdentity` and the root already folds instance names to that identity to admit the graph, so the authority test is a property of the generation rather than a policy judgement. Every other caller is refused with `-4`, including one that holds a participant row of its own. That is deliberate: C8.8 makes *which routes exist* a filtered, per-caller answer the fabric enforces and `sel4_visibility_check` asserts an ungranted caller infers nothing, so serving the raw graph to any caller would bypass that gate rather than implement it. Paged because one record is 128 bytes against a 64-byte message bound; a call answers at most `MAX_STAGED_ARRAY_BYTES / 128` rows (B70). |
