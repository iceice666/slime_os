...
- `just devlog_check` — validate devlog structure, front matter, gates, links, and that every work-item reference resolves. Reads the tree only, so it needs no MyQue binary.
- `just tasks_check` — `myque check` over `.tasks/items/`, then the repository's own policy: backlog-first ordering, backlog-index integrity, legacy-id map integrity, and that a migration-dated closure says its original date is unrecorded.
...
`roadmap/00-backlog.md` is an index into those items, not a second copy of them: a `### B<N> — <title>` heading, the devlog entry holding the investigation, and the UUID carrying the state. The headings survive because 75 devlog entries link into them, so never reword or renumber a landed one. When you close a backlog item, add its heading and `**Item:**` line there; `just tasks_check` fails when a heading names an item the store does not have, or names one that is not closed. Never restate a problem statement, exit condition, or status in that file — the item owns all three.

`roadmap/` remains readable architectural documentation and is no longer authoritative: its headings allocate no identity. `.tasks/legacy-roadmap-ids.json` maps the pre-migration roadmap ids to UUIDs so that devlog entries written before the migration keep resolving; it is committed data, not a parse of `roadmap/*.md`, and nothing new should be added to it.
