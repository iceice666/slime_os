...
Files beginning with `@generated`, files under `boot-contracts/src/generated/`, every manifest named in `DERIVED_GENERATION_FIXTURES` (`scripts/lib/system_spec.py`), the system specs named in `COMPUTED_SYSTEM_SOURCES` (`scripts/lib/system_spec.py`),
and every closure under `contracts/system-image-closure/v2/` are outputs, not sources. Change the matching `contracts/.../schema.zt`, `gen_rust.zt`, or system-spec authoring source (`sources/*.zt` for mapped outputs, otherwise
`systems/*.zti`), then run the matching `scripts/generate/generate-*-bindings.py` / `just *_gen` / `generate-generation-from-spec.py` / `generate-system-image-closures.py`. `components/build-support` separately generates the build-time command tables from the derived `contracts/generation-manifest/v1/fixtures/valid.zti` into each consuming crate's `OUT_DIR`, and copies the per-plane fabric profile the host builder renders.
...
- `just devloop <args>` — the pinned devloop, run against this repository's `deps/zutai` build and [`.devloop/policy.json`](.devloop/policy.json): `admit`, `validate`, `start`, `gate`, `human`, `eligible`, `complete`, `render`, all by canonical UUID. This is how new work is created and closed; `just devloop gate <ITEM> <ACCEPTANCE> --inputs <file>` runs a declared recipe when the acceptance binds the `just-target` gate and `<file>` names it as `{"justTarget": "<recipe>"}`.
...
An item's frontmatter is an open record: MyQue owns an enumerated key set and preserves every other top-level namespace verbatim. `devloop` is this repository's only consumer namespace. An item that carries it is *spec-driven*: its body after the title is exactly one fenced `zti` [`dev-spec/v1`](https://github.com/mozufu/devloop) payload under devloop's `fenced-zti/v1` profile, and its record carries the profile, pinned schema and helper identities, requirements digest, admission identity, and evidence. `just tasks_check` validates those bodies through devloop — which owns that format — while `myque check` still owns store validity. Admit, start, gate, and complete such work with `just devloop`; gate identities and their declared typed observations live in [`.devloop/policy.json`](.devloop/policy.json), never in requirement text. The `just-target` gate is the general one: it takes the recipe name from the execution inputs and reports a single `passed` boolean, so an acceptance can bind to any published `just` target without a bespoke adapter. **A spec-driven body is mandatory for new work.** Every item whose canonical UUIDv7 identity is at or after `2026-10-01T00:00:00Z` must carry a `devloop` record, with no exemption by kind, tag, or state; `just tasks_check` refuses the store otherwise, so create such work with `just devloop admit`, never `myque new`. Items created before that instant — including every legacy `work-item/v1` item — remain valid prose and never become subject, even if reopened. Each devloop validation compiles and links a native binary, so `just tasks_check` costs seconds once an item is spec-driven; a direct `myque close` without `--expected` bypasses devloop's eligibility boundary by design. [`docs/getting-started/06-work-item-lifecycle.md`](docs/getting-started/06-work-item-lifecycle.md) walks both lifecycles command by command, and [`docs/decisions/spec-driven-work-item-bodies.md`](docs/decisions/spec-driven-work-item-bodies.md) records what this repository owns and what it defers.

Backlog defects are the items tagged `backlog`. Resolve, defer, or block every open one before starting a new track milestone; a green verification suite is a precondition for milestone work, not a milestone itself. `deferred` means postponed by decision and `blocked` means waiting on something outside this repository — both satisfy the rule, `open` and `active` do not. `just tasks_check` enforces that ordering and validates the store, and `just tasks_next` lists what is actionable. A new defect is spec-driven like any other new work: admit it with `just devloop admit <spec>.zti --title "<title>" --kind bug` and add `--tag backlog`, then close it with `just devloop complete`. A pre-cutoff defect was created with `myque new` and still closes with `myque close`, which records the closure date; either way record the exit condition that was *observed* in the item's body — a milestone closes on observed evidence, never on implementation status alone.
