...
Milk-V Duo and one injection image, so the AArch64 physical board
`bcm2712-rpi5`, which keeps seL4's 12-bit root CNode default
...
- Observed: `MAX_TASK_ALLOCATIONS` selected 262,657 records for every profile except `slime_cv1800b_duo` and `slime_private_small_tables`, so `aarch64-rpi5` — neither excluded nor re-pinned to a 19-bit CNode — carried tables its kernel cannot pay for; the comment above it named `four_holders_include_static_descriptors_at_default_pool_boundary`, the review that found it, and the ceiling-raising milestone that would fix it.
...
|---|---|---|
| 1 | `sel4/config/qemu-arm-virt.cmake` and `qemu-riscv-virt.cmake` are the only two configs this PR set `KernelRootCNodeSizeBits 19` in; `bcm2712-rpi5.cmake` and `cv1800b-duo.cmake` set it nowhere, and seL4's `config.cmake` defaults it to 12 | The exclusion list and the set of narrow-CSpace platforms disagreed by the AArch64 physical profile |
...
| Physical boot of `aarch64-rpi5` on the fixed image | not run — no board boot was performed for this change | none |
...
- [ ] The Raspberry Pi 5 was not booted for this change, so the fix is verified by measurement against its pinned kernel configuration and installed prefix, not by board evidence. `just rpi5_boot_check` still fails closed on the missing serial adapter.
