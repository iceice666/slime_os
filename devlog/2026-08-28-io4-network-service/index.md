...
**2026-08-29 — The IO4 packet-stack, backend, transfer, reset, restart, and reclamation claims were not observed.**
The frozen body above described Ethernet/ARP/IPv4/ICMP/UDP/TCP/DNS implementation, generation-driven enforcement of every destination bound, a `LinkDevice` loopback provider over IO0 queues, deterministic transfers, backend independence, and numeric reset/restart reclamation as Direct evidence. The reviewed service contained no packet framing or protocol state machine, read none of the eight per-destination ceilings, and did not use its `LINK_SLOT`; the loopback component opened no IO0 queue. The probe's transfer conclusion and four reset/restart/reclamation lines were unconditional strings, while the composition declared no shared buffers, notifications, or supervision from which those effects could occur. Those claims' corrected evidence class is **Unsupported**, not Direct. The authority-table request/denial behavior that the probe and intruder actually exercise remains the only direct plane evidence, subject to the current roadmap's narrower status.

The decision that DHCP, SLAAC, and NDP were already declared and refused is also withdrawn: the current contracts do not declare those facilities, so adding them would require contract work rather than only an implementation change. The roadmap's explicit unimplemented list is authoritative.

**2026-08-29 — The network-service binding drift guard is now registered.**
The original regression-guard row named `generate-network-service-bindings.py --check` "via `just contracts_check`" before that registry actually invoked it. The registry now type-checks `contracts/network-service/v1/schema.zt` and runs the generator's `--check`; the same registration was added for `LinkDevice`, with `network_service_gen` and `link_device_gen` recipes. `python3 scripts/check/check-contracts.py` now exits 0, prints both `LinkDevice protocol bindings are current` and `Network service protocol bindings are current`, runs all 310 boot-contract tests successfully, and includes both protocols in its aggregate pass line.

**Disposition of the frozen verification rows.** The `Granted path observed` row is Direct only for protocol-level authority admission and replies; its "transfer"/"echo" wording did not observe bytes crossing a packet or link backend, and the DNS line observed authorization/reply handling rather than DNS framing or resolution traffic. The `Reclamation observed` row is **Unsupported**, not Direct. The loopback/backend decision and the open-risk statements claiming a bounded TCP state machine, declared DHCP/SLAAC/NDP, or proof against a loopback backend are superseded by the roadmap: none of those mechanisms was present in the reviewed artifact.

The loopback fixture was then reduced to its truthful role: it resolves the one declared endpoint binding and computes `[io-link-loopback] declared endpoint bindings=1 protocol operations=0`. Its dead contract-reference function and artificial `slime-proto` dependency were removed. The final service markers compute five destinations; declared `socket_limit=7 listener_limit=0 dns_record_limit=2`; and observed request, packet, socket-refusal, DNS-refusal, and cross-holder-refusal counts. Probe and intruder markers compute successful capability operations, exact-authority refusals, DNS/socket budget refusals, cross-holder refusals, and rights-mask refusals. Socket, listener, and DNS-record ceilings are now enforced from generation data, but this does not add packet framing, byte transfer, a link backend, reset/restart reclamation, or enforcement of the other five decoded traffic/state ceilings.

**2026-09-11 — A data plane now exists behind this boundary, on a different plane.**
IO11 ([`../2026-09-11-io11-link-attach/`](../2026-09-11-io11-link-attach/index.md), [`../2026-09-11-io11-tcp-client/`](../2026-09-11-io11-tcp-client/index.md)) runs smoltcp inside `network-service` on the new `sel4-io-tcp` composition (generation 54), bound to the IO3 virtio-net driver: ARP, IPv4, ICMP echo reply, and a TCP client byte stream to one declared destination are observed there. Nothing in this entry's frozen body or its earlier corrections changes: generation 53 remains authority-only, its baseline frozen, and `just io_network_check` still asserts no transfer on it. The withdrawn claims stay withdrawn for this plane; UDP, DNS resolution, listen/accept, reset/restart reclamation, and backend independence remain unobserved everywhere.
