...
| Root (board fix) | The NS02201 reset-granule mapping moved above the product-input block: the clock-gate (`0x2f0020000`) and watchdog (`0x2f0060000`) sit below UART0 (`0x2f0130000`) in one device untyped, and device untypeds retype monotonically upward — the invariant the Duo's `RESET_PAGE` comment states, violated here only by a product image | A product image keeps its reset path; the carve order is stated at the mapping site |
| Gate (board fix) | `read_until` fails naming the failure marker it saw instead of returning for a later assertion to trip | A root FATAL reads as its own diagnosis, not as whichever count check runs next |
