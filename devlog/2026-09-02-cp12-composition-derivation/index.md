...
| `contracts/system-image-closure/v1/schema.zt` | Added `NegativeBuildCase` — a separate record type naming a base closure by identity, one of six closed B40 mutations, and the refusal its audit must emit; `build-system-image.py --negative` writes no image-identity or build-result record | A deliberately invalid root cannot be presented as a product image, and the ambient `SLIME_B40_MUTATION` no longer reaches a closure build |
...
- [ ] CP14's fifth deliverable: QEMU disks, device topology, corruption
      schedules, and runtime fault controls have not moved into
      `system-test-run/v1`. The contract has fields for them and one record uses
      them, but the 36 plane checkers still own their fixtures directly, and
      moving them is coupled to CP15's checker migration.
