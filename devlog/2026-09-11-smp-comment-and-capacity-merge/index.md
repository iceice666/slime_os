...
| Kind | Change |
| Status | Verified |
| Scope | QEMU ARM SMP configuration comments, private-memory endpoint ABI slots, allocator comment ownership, aligned live-untyped capacity simulation, main integration, generated image closures and test-run identities |
| Work items | none |
| Gates | `just system_spec_check`, `just test_sel4_root`, `just private_memory_check`, `just system_test_run_check`, `just fmt_check_all`, `just lint_all`, `just devlog_check` |
| Trigger | PR #25 reviews requested moving implementation history out of configuration and allocator comments, pinning the denied private-memory probe endpoint to its hard-coded ABI slot, and refusing capacity reports whose aggregate ordinary bytes cannot realize the aligned extent sequence; the branch also conflicted with main |
| Baseline | Capacity branch `640a6cba`, review-fix merge `b9d4027f`, main `a1119ca6`, and pre-alignment review head `12d73649` |
...
realize. Integrated main without reverting the branch's closure-provenance
cutover or 19-bit QEMU root-CNode pins.
