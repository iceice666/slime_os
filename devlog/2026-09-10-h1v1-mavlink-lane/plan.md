...
"Radio on GPIO" means the SoC's UART8, whose transmit pad is P_GPIO[4] on the same 3.3 V bank
the ESC probe already found on the 40-pin header. Nothing is bit-banged.
...
| S3 | `feat/nt98690-mavlink` | Board: UART8 carve and clock/pinmux bring-up from pins, build inputs, the board gate whose exit is decoded on the ground radio | `P6.E` (board) |
...
UART8 TX and a second RFD900x on the development host: the resident Slime graph emits one
...
UART8 clock, reset, divider, pinmux and pad words read their surveyed values; the port takes the
...
This table adds only what UART8 needs.

| Fact | Value | Source |
|---|---|---|
| UART8 is a 16550 | `uart@2,f0137000`, `compatible = "ns16550a"`, `reg = <0x2 0xf0137000 0x0 0x1000>`, `reg-shift = 2`, `reg-io-width = 4`, `clock-frequency = 48000000`, `baud = 115200` (DT default, unused by Slime), `hw_flowctrl = 0` | decompiled board DTB `nvt-evb.bin` (`/srv/novatek/sdk/worktrees/lamb-h1v1/output/`), node at line 268 of the scratch `h1v1.dts` |
| Interrupt | `interrupts = <0 0x60 4>` → SPI 96 → GIC INTID 128 (UART0 is SPI 43, INTID 75). Unused by a TX-only lane | same DTB, lines 268/157 |
| UART8 clock | DT `clk_uart7`: `div_reg_offset 0x60`, `div_bit_idx 16`, `div_bit_width 8`, `gate_reg_offset 0x7c`, `gate_bit_idx 23`, `reset_reg_offset 0x9c`, `reset_bit_idx 23`, `current_rate 48000000`, parent phandle 0x106 | same DTB, `clk_uart7` node |
| Naming trap | DT `uart7` / `clk_uart7` at `0x2f0137000` **is** the SoC's "UART8" (the pinmux and U-Boot name). DT `uart8` at `0x2f0138000` is SoC UART9. This plan uses the SoC name UART8 throughout | `configs/.../nvt-peri.dtsi:23-24,145-173`; U-Boot `soc.c:286-308` (below) |
| CG base | `0x2_F002_0000` (already `reset_cg_base` in pins) | `u-boot/arch/arm/include/asm/arch-nvt-ns02201_a64/IOAddress.h:20,40` |
| UART8 clock source | parent phandle `0x106` = `fix480m`, **480 MHz** fixed | `h1v1.dts:2730-2735`; `configs/.../nvt-clock.dtsi:92-97` |
| UART8 clock divider | `CG+0x60` bits **[23:16]** (`CG_UART_CLK_DIV_REG1_OFFSET`); the field holds **(divisor − 1)**: rate = 480 MHz / (field + 1). For 48 MHz the field is **9**. Its value at the U-Boot prompt is **not established on disk** (no U-Boot or TF-A code writes `CG+0x60`; the loader is a blob) — the probe writes 9 rather than trusting what it finds, and restores the surveyed value afterwards | `linux-kernel/include/dt-bindings/clock/nvt-ns02201.h:80`; `nvt-clock.dtsi:1070-1078`; `drivers/clk/novatek/nvt-clk-provider.c:912-915` (set), `:857-862` (recalc) |
| UART8 clock gate | `CG+0x7C` bit **23** (`CG_CLK_EN_REG3_OFFSET`); **1 = clock on** | `nvt-ns02201.h:89`; `nvt-clock.dtsi:1076-1077`; `nvt-clk-provider.c:726-733` |
| UART8 reset | `CG+0x9C` bit **23** (`CG_SYS_RESET_REG3_OFFSET`); **active-low RSTN: 1 = released, 0 = held in reset**. The DT says `do_reset = NOT_RESET`, so Linux only ever sets the bit; the probe does the same (set if clear, never pulse) | `nvt-ns02201.h:107`; `nvt-clock.dtsi:1078-1080`; `nvt-clk-provider.c:651-666, 687-694` |
| Pinmux for UART8 → P_GPIO[4..5] | `TOP_REG13_OFS = 0x34` ("TOP Control Register 13 (UART[1/2])"), field `UART8` = bits **[31:28]**; `MUX_1 = 1` routes UART8_1 = P_GPIO[4] TX, P_GPIO[5] RX; `MUX_2 = 2` routes C_GPIO[17..18]; 0 = unrouted. `TOP_REG14_OFS = 0x38`, field `UART8_RTSCTS` = bits **[19:18]** — **never written** (no flow control) | `linux-kernel/drivers/soc/nvt/plat-novatek/include/plat-ns02201_a64/top_reg.h:20-22, 294-328`; `include/dt-bindings/pinctrl/nvt-ns02201-pinctrl.h:650-651, 662-663`; `ns02201_pinmux_host.c:7167-7212` |
| What the Linux pinmux driver writes for UART8_1 | **Exactly three fields**: `TOP+0x34[31:28] = 1`, `TOP+0xA8` bit 4 = 0, `TOP+0xA8` bit 5 = 0. No pad, pull, drive, or other register | `ns02201_pinmux_host.c:7167-7212, 11552-11565` |
| The vendor's own U-Boot recipe for UART8_1 (never compiled in, but authoritative) | `TOP+0xA8 &= ~0x30` (bits 4,5 → FUNCTION); `TOP+0x34 = (v & ~0xF0000000) \| 0x10000000` (UART8 = MUX_1); `CG+0x7C \|= 1<<23` (gate on). Then the generic ns16550 init | `u-boot/arch/arm/mach-novatek/nvt_ns02201_a64/soc.c:286-308` (`serial_preinit`, called from nowhere in the tree) |
| Package pins | P_GPIO4 = `UART8_1_TX` (O), P_GPIO5 = `UART8_1_RX` (I), P_GPIO6/7 = RTS/CTS (optional); all `3318 IO` (3.3 V), default pull-down; INT[62..65] | pinmux CSV rows 111–114 |
| Pad word for P_GPIO[4] | `TOP+0xA8` (`TOP_REGPGPIO0_OFS`, one bit per pad, `PGPIO_4` = bit 4, `PGPIO_5` = bit 5; `0 = FUNCTION`, `1 = GPIO`). **Only bit 4 is cleared**: the TX pad is routed, P_GPIO[5] stays GPIO (no RX). With the RX function input unrouted the 16550 may report `LSR.BI`/`DR`; the probe and driver mask LSR to THRE (bit 5) and TEMT (bit 6) | `top_reg.h:35-41, 509-544` |
| Pad electrical for P_GPIO[4] | Pull: `PAD+0x08` bits [9:8] (`PAD_PIN_PGPIO4`; 0 none, 1 down, 2 up, 3 keeper; board default down). Drive: `PAD+0x110` bits [19:16]. Rail: `PAD+0x210` bit 4 = P1 rail, 0 = 3.3 V, board declares `pgpio_0_19` on P1 at 3.3 V; observed `0x00000024` at the prompt (3.3 V). **None of these is written** — a push-pull UART TX needs no pull, and the ESC probe already asserts the rail bit | `plat-ns02201_a64/pad.h:22-57, 111-118, 302-309, 479`; `ns02201_pad_host.c:66-75, 141-161, 405-410`; `configs/.../nvt-peri-dev.dtsi:8`; ESC audit entry |
| GPIO bank words for P_GPIO[4] | Same bank-1 words the PWM probe uses: DATA `GPIO+0x04`, DIR `+0x34`, SET `+0x64`, CLR `+0x94`, bit 4. The second word starts at P_GPIO[32], so `--gpio-probe 4` needs no change | `plat-ns02201_a64/nvt-gpio.h:20-23, 38, 47`; `drivers/gpio/gpio-nvt.c:433-447, 539-559` |
| Header pin for P_GPIO[4] | **unknown on disk** — no schematic, pinout table, or header note exists under the SDK, the companion repo, or the docs; the ESC run found P_GPIO[0] on the 40-pin header and explicitly did not record the position. **S1 finds it with `--gpio-probe 4`.** | searches over `~/nt98690-ubuntu`, `/srv/novatek/sdk/docs`, `/srv/novatek/sdk/worktrees/h1v1-dev`; `devlog/2026-09-08-h1v1-pwm-probe/index.md:17-18, 41` |
| U-Boot and UART8 | U-Boot's serial is the generic `ns16550.c` with `MEM32` (4-byte stride), `COM1..COM5` = `0x2f0130000..0x2f0134000` only, `CONS_INDEX = 1`, `SYS_NS16550_CLK = 48000000`. No `COM6..9`, so no code path touches UART8; `serial_preinit` is never called; U-Boot's pinmux host is compiled only with `CONFIG_VIDEO_LOGO` (unset). Linux's `uartII{pinmux = <0x0>}` routes no UART6–9 pad. **Expect the gate off, the reset either state, the mux 0, and bits 4/5 of `TOP+0xA8` set at the prompt — unobserved; S1 reads them** | `include/configs/novatek/ns02201_a64.h:19-29`; defconfig `:253, 1180-1193, 209`; `nvt_ns02201_a64/Makefile:14`; `configs/.../nvt-top.dtsi:21-22` |
...
| Line settings | 8N1, no flow control: `LCR = 0x83` (DLAB) → `DLL = 52`, `DLM = 0` → `LCR = 0x03`; `FCR = 0x07`; `MCR = 0x00` (U-Boot writes `0x03`; DTR/RTS reach no routed pad, so either is inert — `0x00` is pinned so the value is not mistaken for flow control); `IER = 0x00`. Readback: `LCR`, `DLL`/`DLM` (with DLAB), `MCR`, `IER`; `FCR` is write-only and `IIR` reads `0xC1` once FIFOs are enabled | 16550 register map; U-Boot sequence above; RFD900x default serial format |
...
| Core rail | UART8's words (`CG+0x60[23:16]`, `CG+0x7C` bit 23, `CG+0x9C` bit 23, `TOP+0x34[31:28]`, `TOP+0xA8` bit 4) are disjoint from the PWM12 set (`CG+0x8C`, `CG+0xA4`, `CG+0x324`, `TOP+0x1C`, `PWM+0x60/0x64`, `PWM+0x104` bit 12) and from the PWM0 set (`CG+0x30`, `CG+0x84`, `TOP+0x18`, `TOP+0xA8` bit 0). The probe still asserts `CORE_RAIL_INVARIANTS` before its first write and after its last, because CG and TOP are shared pages, and every write is a read-modify-write of one field | ESC plan A2; the offsets above |
| Write list, in order (all RMW with readback) | 1. `CG+0x9C \|= 1<<23` (release reset if held) 2. `CG+0x60[23:16] = 9` 3. `CG+0x7C \|= 1<<23` 4. `TOP+0x34[31:28] = 1` 5. `TOP+0xA8 &= ~(1<<4)` 6. 16550 init on `0x2_F013_7000` (A2 line settings). Restore is 5 → 1 in reverse, each read back | derived from the rows above; vendor `serial_preinit` is the same sequence minus the reset and divider it never had to touch |
...
| Wiring | Board P_GPIO[4] (UART8 TX) → radio RX; board GND ↔ radio GND; radio VCC from its own 5 V supply; P_GPIO[5] not connected (no RX). Radio RX is a 3.3 V input, so no level shifting | pinmux CSV (3.3 V bank); radio datasheet |

## A4. Fixed decisions and names

| Decision | Choice | Rejected |
|---|---|---|
| The port | UART8 at `0x2f0137000`, TX on P_GPIO[4], **TX pad only routed** (P_GPIO[5] stays GPIO, RTS/CTS untouched) | UART0: it is the kernel debug console and the Slisp input path; a second radio on it would interleave with the shell. Bit-banging a GPIO at 57600 from seL4 userspace: no timing guarantee |
| Line config ownership (S2/S3) | The **driver** programs LCR/DLL/DLM/FCR/MCR on its own mapped page; the **root** programs the SoC-wide words once at carve time (`CG+0x9C` release, `CG+0x60` field = 9, `CG+0x7C` gate, `TOP+0x34` mux, `TOP+0xA8` bit 4) and prints `SLIME_NT98690 uart8 bringup pad=P_GPIO4 clk_hz=48000000` — the PWM lane's split, for the same reason (SoC-wide pages never reach a component) | Mediated driver writes to CG/TOP |
...
| Names | Contracts `contracts/serial-device/v1`, `contracts/mavlink-heartbeat/v1`; components `components/services/uart16550-driver`, `components/applications/mavlink-heartbeat`; composition `sel4-mavlink`; markers `SLIME_NT98690 uart8 bringup …`, `[uart16550-driver] ready clk_hz=48000000 divisor=52`, `[mavlink-heartbeat] send seq=<n> status=<ok\|no-device\|device-error\|timeout>`; gate `nt98690_mavlink_check`; pins keys `uart8_*` in `[ns02201_h1v1]`; work items `P6.MAV` (epic), `P6.MAV.A` (bench), `IO9` (mechanism), `P6.E` (board) | `nvt-uart-driver` (the driver is a plain 16550, not Novatek-specific — the SoC facts stay in the root) |
...
- Pins checker `scripts/check/check-sel4-pins.py` @3429307f: H1V1 block inside `check_profile`
  ...
  :597 gating `pwm_channel/pwm_pad/pwm_pad_header/pwm_probe_observed` :600-623. `uart8_*`
  keys and an `observed_uart_routes` table follow that pattern. **No key for UART8 or
  `0x2f0137000` exists anywhere in the tree today.**
...
- H1V1 carve block `main.rs:787-826` under `#[cfg(slime_ns02201_h1v1)]`, invariant comment
  ...
  and prints `SLIME_ROOT product input ready uart=…` :864. UART8 at `0x2_f013_7000` is above
...
- `device.rs`: `DeviceRegion::{map :144, map_child :119, read32 :209, write32 :220,
  ...
  **the root never programs a divisor or LCR** — firmware owns UART0's line config. UART8 has no
...
1. **Which header pin follows P_GPIO[4]** (`--gpio-probe 4`, with the pad-level readback the review round added). No-go if none: fall back to the UART8_2 route on C_GPIO[19..20] (`MUX_2`, pinmux CSV rows 27–28, also 3.3 V) — only if those reach a connector, else a soldered lead.
2. **The surveyed state at the prompt**: `CG+0x60`, `CG+0x7C`, `CG+0x9C`, `TOP+0x34`, `TOP+0x38`, `TOP+0xA8`, `PAD+0x08`, `PAD+0x210`, and UART8 `LCR/LSR/IER/MCR/IIR` — pinned as `uart8_*_at_prompt` keys so S3's root asserts what it found rather than assuming.
...
- **Shared carve block with the ESC lane's S3**: both grow the same carve loop; the second to land rebases. Ordering stays ascending: TOP → CG → WDT → PWM → UART0 → UART8.
- **Core rail**: UART8's words are disjoint from PWM12's, but they share pages; the probe keeps the rail invariants and read-modify-writes one field per word.
...
**S3 (board).** Pins → `build-sel4.py` exports `SLIME_NS02201_UART8_PADDR`, `SLIME_NS02201_UART8_CLK_HZ`, `SLIME_NS02201_UART8_DIVISOR`, `SLIME_NS02201_UART8_PAD` under `--mavlink-graph` on the H1V1 → `build.rs` cfg `slime_ns02201_uart8` → root carve appends UART8 after UART0, one-time CG/TOP bring-up with readback, static region handed to the declared inventory. Gate `scripts/check/check-nt98690-mavlink.py` in the Slisp checker's shape plus a second serial port for the ground radio: boots, waits for the ready and first-send markers, decodes ≥ 10 heartbeats on the radio port, checks cadence, sends the terminator, expects the banner. Evidence: UART0 transcript, radio-port capture, decoded-frame table, identities.

---

# Part B — Session 1: bench probe and facts (no Slime code)

Goal: from the unmodified vendor `nvt:` prompt, find P_GPIO[4]'s header pin, survey UART8's
clock/mux/line state, transmit pinned heartbeats through UART8 into an RFD900x, decode them on
...
- Two RFD900x radios, assumed paired (A3). The ground radio on the host's second USB-serial port
  (`/dev/ttyUSB1` below); the air radio on its own 5 V supply. Only P_GPIO[4] → air-radio RX and
  GND ↔ GND reach the board (3.3 V bank, no shifter). P_GPIO[5], RTS, CTS unconnected.
...
1. **Work items.** `myque new` four items (UUIDs allocated by the tool; display keys per A4):
   ...
   `architecture`, `hardware`); milestone `P6.MAV.A` "H1V1 UART8 bench probe: heartbeats
...
3. **Bytes from the console** — `scripts/lib/uboot_console.py`: `Console.read_bytes_for(self,
   ...
   states that invariant. *Choice:* the radio port is a second `Console(receiver, UART8_BAUD,
...
5. **Constants, pure helpers, pins** — `scripts/check/check-nt98690-boot.py` beside the PWM
   block, each cited to A2: `UART8_BASE = 0x2_F013_7000`, `UART8_REG_SHIFT = 2`,
   `uart8_register(index: int) -> int` (`UART8_BASE + (index << UART8_REG_SHIFT)`, the one
   ...
   compared outside these two bits), `UART_TEMT_POLLS = 5`; `UART8_CLOCK_SOURCE_HZ =
   480_000_000`, `CG_UART8_CLK_DIV = 0x60`, `CG_UART8_CLK_DIV_SHIFT = 16`,
   `CG_UART8_CLK_DIV_MASK = 0xFF << 16`, `CG_UART8_CLK_DIVIDER = 9` (field = divisor − 1),
   `UART8_CLOCK_HZ = UART8_CLOCK_SOURCE_HZ // (CG_UART8_CLK_DIVIDER + 1)`,
   `CG_UART8_CLK_EN = 0x7C`, `CG_UART8_CLK_EN_BIT = 23`, `CG_UART8_RESET = 0x9C`,
   `CG_UART8_RESET_BIT = 23` (active-low RSTN: 1 = released; set if clear, never pulsed);
   `TOP_UART8_MUX = 0x34`, `TOP_UART8_MUX_MASK = 0xF << 28`, `TOP_UART8_MUX_1 = 1 << 28`,
   `TOP_UART8_RTSCTS_MUX = 0x38` (bits [19:18], survey only, never written),
   `UART8_PAD_BIT = 4` (`TOP_PGPIO_FUNC`), `PAD_PGPIO4_PULL = 0x08` (bits [9:8], survey only);
   `UART8_BAUD = 57_600`, `UART8_DIVISOR = 52`; `UART_PROBE_FRAMES = 10`,
   `UART_PROBE_INTERVAL_SECONDS = 1.0`, `UART_PROBE_RECEIVE_WINDOW_SECONDS = 2.0`. Pure
   helpers with the documented example asserted: `uart8_clock_hz_from_field(field: int) -> int`
   (`480 MHz // (field + 1)`; `9 → 48_000_000`; used only to *print* what the prompt-time field
   means), `uart8_divisor(clock_hz: int, baud: int) -> int` (`round(clock / (16 × baud))`;
   `ValueError` beyond 2 % rate error; `(48_000_000, 57_600) → 52`),
   `cg_uart8_divider_word(current: int) -> int` (field replaced by 9).
   `check_uart_pins(profile) -> None` mirrors `check_pwm_pins`: the bases plus
   `uart8_reg_shift`, `uart8_reg_io_width`, `uart8_clock_source_hz`, `uart8_clock_divider`,
   `uart8_clock_hz`, `uart8_baud`, `uart8_divisor` must equal the constants.
   `sel4/pins.toml [ns02201_h1v1]` after the PWM keys: `uart8_base = "0x2f0137000"`,
   `uart8_reg_shift = 2`, `uart8_reg_io_width = 4`, `uart8_clock_source_hz = 480000000`,
   `uart8_clock_divider = 9`, `uart8_clock_hz = 48000000`, `uart8_baud = 57600`,
   `uart8_divisor = 52`, with a comment that the observed keys are absent until the probe
   reports them. `check-sel4-pins.py` after the PWM block: `uart8_base == 0x2_F013_7000`,
   shift 2, width 4, source exactly 480 MHz, `uart8_clock_hz == source // (divider + 1)` with
   `divider == 9`, `uart8_divisor == round(uart8_clock_hz / (16 × uart8_baud)) == 52` (a
   self-consistent 24 MHz/26 pair fails), and `observed_uart_routes: dict[int, tuple[str, str,
   str]] = {}` gating `uart8_pad`, `uart8_pad_header`, `uart8_clock_at_prompt`,
   `uart8_mux_at_prompt`, `uart8_probe_observed` exactly as `observed_pwm_routes` does — all
   absent while the table is empty, all present and matching once it holds `{8: ("P_GPIO4",
   "<header pin>", "2026-09-XX")}`. `uart8_clock_at_prompt` and `uart8_mux_at_prompt` are
...
6. **The probe mode and its host model.** CLI in `main()` :1293: `--uart-probe`
   ...
   `--uart-receiver` (str, default `None`; tty or `tcp:HOST:PORT`, opened at `UART8_BAUD`),
   ...
   - `uart_survey(console, prompt) -> dict[str, int]` over `UART_SURVEY_REGISTERS`
     (`cg_uart8_divider`, `cg_uart8_clock_enable`, `cg_uart8_reset`, `top_uart8_mux`,
     `top_uart8_rtscts_mux`, `top_pgpio_func`, `pad_pgpio4_pull`, `pad_p1_status`), printing
   ...
   - `program_uart8_line(console, prompt, divisor: int) -> None`, U-Boot's `ns16550_init`
     ...
     `verify_register(IIR, 0xC1, mask 0xC1)`; LCR=0x03; LCR=0x83; DLL=`divisor & 0xFF`;
   ...
   - `uart_probe(console, prompt, frames, interval, receiver, window, divisor, timeout) -> str`:
     ...
     preserved): RMW `CG_UART8_RESET` set bit 23 if clear; RMW divider field to
     `CG_UART8_DIV_FIELD_48MHZ` (unconditionally — A4); RMW `CG_UART8_CLK_EN` set bit 23;
     `uart_survey_port`; RMW `TOP_UART8_MUX` field = 1; RMW `TOP+0xA8` clear bit 4 (bit 5
     untouched, TOP14 never written); `program_uart8_line`; drain the receiver 0.5 s; per frame
   ...
   - Host model in `check-nt98690-bench-probe.py`: `class Uart16550Model` (`dll, dlm, lcr,
     ...
     `ModelConsole._execute` for any address in the UART8 page: THR writes append to `tx` (DLL
     when DLAB), LSR reads `0x71` (THRE|TEMT plus BI|DR, so an unmasked compare fails) or
     `0x11` when `temt_stuck`, DLL/DLM/LCR/IER/MCR read back, IIR reads `0xC1` after FCR bit 0,
     FCR write-only, index-0 reads raise `AssertionError("RBR popped")`; reads of the UART8 page
     ...
     UART8 held), `TOP+0x34 = 0`, `TOP+0x38 = 0`, `PAD+0x08 = 0x100`. `class ModelReceiver`
     ...
     commands in U-Boot order, no `md.l` at index 0, no `mw.l` at TOP+0x38, `TOP+0xA8` bit 5
     ...
     `reset_already_released_untouched` (bit 23 set at survey: no `mw.l` to `CG+0x9C` at all);
     ...
     `reset_release_dropped_refuses` (a dropped RMW on `CG_UART8_RESET` fails naming the
     register, no divider/gate/UART8 write follows, cleanup still runs);
     `dropped_restore_detected` (parametrised over the five restored words: failure names
     `restore UART8 <word>`, later steps still verified, `reset` still attempted);
     ...
     restored); `uart_pins_bind_the_divisor` (`check_uart_pins` refuses `uart8_divisor = 26`,
     `uart8_clock_hz = 24000000`, `uart8_clock_divider = 19`, `uart8_baud = 115200` one at a
...
8. **Docs and gates.** Module docstring names the new mode and the never-touch set (TOP14,
   UART8 index 0, the PWM12 words); the `just/hardware.just` comment mentions the 16550 model.
...
| 1 | Power cycle 1. `--survey`, then `--gpio-probe 4 --transcript …/gpio-probe-4.log`, meter on the 40-pin header (orient from the P_GPIO[0] position if the ESC rerun records it this session) | **which header pin follows P_GPIO[4]**; level readbacks in the transcript |
| 2 | Board off. Wire P_GPIO[4] → air-radio RX, GND ↔ GND; radio on its own 5 V. Power cycle 2 | — |
...
- `sel4/pins.toml`: `uart8_pad = "P_GPIO4"`, `uart8_pad_header = "<pin>"`,
  `uart8_clock_at_prompt = { divider, gate, reset }`, `uart8_mux_at_prompt = { top13, top14,
  pgpio_function }`, `uart8_probe_observed = "2026-09-XX"`; `observed_uart_routes` gains
  `{8: ("P_GPIO4", "<pin>", "2026-09-XX")}`; `just sel4_pin_check`.
- a dated `h1v1-uart-probe` devlog entry, Kind `Audit`: `uart-probe.log` (UART0 raw),
  ...
  immutable capture), `gpio-probe-4.log`; an Investigation-log row per B2 step; the `ATI5`
...
- A core-rail invariant fails at survey, or `PAD+0x210` bit 4 reads 1 → stop, write nothing,
  record the values.
- U-Boot prints `"Synchronous Abort" handler` on the first UART8 read → the gate or reset
  polarity is not what A2 says; power-cycle, record the CG words, no retry with guessed bits.
- No header pin follows P_GPIO[4] → record; fall back to UART8_2 on C_GPIO[17..18] (`MUX_2`)
  only if those reach a connector, else a soldered lead — a new route entry with its own run,
  never a widened range.
...
| Future combined image | `declared(region)` fills `regions[0]`/`devices[0]`, `len = 1`. A `sel4-pwm-mavlink` would add `AuthorityInventory::declare_next(&mut self, region) -> Result<usize, DeviceRegion>` filling `regions[len]`/`devices[len]` up to `MAX_IO_DEVICES = 2` (exactly PWM + UART8), ordinals in ascending paddr (PWM `device 0`, UART8 `device 1`), two budget rows. Not built by either lane | Widening `MAX_IO_DEVICES` |
...
`init-mavlink-heartbeat` (executable, `["exec"; "spawn"]`), `uart8-device` (device,
`["mapMmio"]`, source/target driver), `uart8-mmio` (mmioRegion, `["mapMmio"]`),
...
`heartbeat-serial-rpc 0 / uart8-device 1 / uart8-mmio 2` `componentAbi`, heartbeat
...
Branch `feat/nt98690-mavlink`. Depends on S1 (pins `uart8_*`) and S2. Uses A2's concrete words.
...
MAVLINK_VARIANT`: root env `SLIME_NS02201_UART8_PADDR`, `SLIME_NS02201_UART8_CLOCK_HZ`,
`SLIME_NS02201_UART8_DIVISOR`; generation env (the `environment=` argument of
`build_sel4_generation`, `:760-778`) `SLIME_UART16550_CLOCK_HZ`, `SLIME_UART16550_DIVISOR`.
`slime-root/build.rs`: `rerun-if-env-changed` ×3, `rustc-check-cfg=cfg(slime_ns02201_uart8)`,
`rustc-cfg=slime_ns02201_uart8` when the paddr is set (refuse unless `target_profile ==
"aarch64-sel4-nt98690-h1v1"`), `rustc-env` pass-through of the three.

**Root** (`slime-root/src/main.rs`, under `all(slime_ns02201_h1v1, slime_ns02201_uart8)`):
statics `NS02201_UART8_PAGE: FreePage`, `NS02201_UART8_REGION: Option<device::DeviceRegion>`,
`NS02201_UART8_PADDR = const_parse_hex_usize(env!(…))` (`:422`); the TOP granule
`NS02201_PINMUX_PAGE` + `NS02201_PINMUX_REGISTERS: Option<MappedGranule>` shared with the ESC lane
under `any(slime_ns02201_pwm, slime_ns02201_uart8)`, carved first in the `:791-825` loop (TOP
...
NS02201_UART8_PADDR)` → static. `ns02201_uart8_bringup(cg: &MappedGranule, top:
&MappedGranule)`, called right after the carve, all RMW with readback, in A2's order: (1)
`CG+0x9C |= 1<<23` (active-low reset; set if clear, never pulse), (2) `CG+0x60[23:16] = 9`, (3)
`CG+0x7C |= 1<<23`, (4) `TOP+0x34[31:28] = 1`, (5) `TOP+0xA8 &= ~(1<<4)` (bit 5 untouched); any
readback mismatch → `fatal!("uart8 bringup readback reg=… want=… got=…")`; then
`SLIME_NT98690 uart8 bringup pad=P_GPIO4 clk_hz=48000000`. Pure helpers in `platform_timer.rs`
beside the ESC lane's: `uart8_divider_word(current, field) -> u32`, `uart8_mux_word(current) ->
...
`#[cfg(any(slime_ns02201_pwm, slime_ns02201_uart8))]`: `match (PWM.take(), UART8.take()) {
...
lands second rebases onto this shape. The `sel4` H1V1 image (no `slime_ns02201_uart8`) keeps its
exact carve order.
...
allocator, `SLIME_NT98690 uart8 bringup pad=P_GPIO4 clk_hz=48000000`, `SLIME_ROOT product input
...
the operator's counted steps in the comment (1 card carries the `.bin`; 2 P_GPIO4 → radio RX,
...
**Records**: `roadmap/07-architecture-portability.md` `### P6.E — MAVLink heartbeat over UART8
...
- **Mediated path on a declared region**: `read_mmio32` indexes `inventory.region(…)` and reads
  at the root VA; if any later code `map_child`s or `unmap`s the UART8 region, every driver access
...
- **Shared carve block**: ascending order TOP → CG → WDT → (PWM) → UART0 → UART8; a misordered
  UART8 carve reproduces P6.C's `Allocate(DeviceFramePassed)`. `nt98690_slisp_check` must stay
