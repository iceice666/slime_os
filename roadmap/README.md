...
| --- | --- | --- |
| [Backlog](00-backlog.md) | B1–B69 and B71–B76 resolved; **B70 open** (component definitions and slot/route bindings are compile-time-coupled to one crate's private manifest parser) — CP0/CP1 landed the specification and derivation halves, CP2 landed the query, and 2026-08-21's boot action took the `include!` surface from 15 sites to 9 | B70's remainder is 9 `fabric_profile` sites whose symbols size fixed arrays at compile time, so no query retires them; they are CP3/CP4 declared-capacity work in the [Component platform track](10-component-platform.md). B61/B63/B65 each record a deliberately deferred half a future audit should start from |
...
| [Core runtime](02-core-runtime.md) | C7 and all of C8 (C8.1–C8.15) complete; the C8 track closed 2026-08-17 with C8.14's fault-isolation envelope and C8.15's aggregate determinism gate (279 trace records across four boots, compared per worker and record kind after B68 found the flat comparison was asserting one scheduling interleaving, and per declared field after B75 found three of them were observations of the run rather than of the composition) | C9 robot runtime authority, then C10 private component memory |
| [Component platform](10-component-platform.md) | CP0, CP1, CP3, and CP4 complete; CP2's root-served query resolves grant bindings, namespaced boot-layout roles, capability roles, the fabric graph, and the generation's boot action; CP5 not started | CP4 closed 2026-08-21: component specs explicitly select workspace or content-hash-bound external implementations, and the generation builder rejects malformed external ELFs before whole-generation signing. Next is CP5's proof from a distinct git checkout against the SDK boundary. B70's `include!` surface stays at 9 sites, all `fabric_profile` readers whose symbols size fixed arrays at compile time — not closable by any query, so they remain CP5 declared-capacity work |
