...
| [Component platform](10-component-platform.md) | CP0, CP1, and CP3 complete; CP2's root-served query resolves grant bindings, namespaced boot-layout roles, capability roles, the fabric graph, and the generation's boot action; CP4–CP5 not started | CP3 closed 2026-08-21: `components/bins` is 52 independent crates over a shared `slime-components` library and a documented `slime-build-support` parser, with the allocator scoped to the six crates that declare it. Next is CP4's external-artifact admission path. B70's `include!` surface stays at 9 sites, all `fabric_profile` readers whose symbols size fixed arrays at compile time — not closable by any query, so they are CP4/CP5 declared-capacity work |
