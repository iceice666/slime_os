...
| [Architecture portability](07-architecture-portability.md) | P0, P1, P2.1, P2.2, P3.D, and P5 complete; P2.3–P2.6 superseded by P5. Milk-V Duo is the current physical execution lane; its payload handoff loop is observed, while P3 RV64 QEMU and P3.E seL4-on-Duo remain open | P3 RV64 QEMU corpus, then P3.E risk gates and physical seL4/`slime-root` boot on the Duo |
...
The active lane is now P3/P3.E in the [Architecture portability track](07-architecture-portability.md). Work should close these risks in order:

1. **RV64 reference profile:** boot the pinned `riscv64-sel4-qemu-virt` profile and replay the architecture-neutral isolation, wait/wake, sample-plane, generation, and rollback corpus.
2. **Duo memory fit:** measure elfloader, kernel, root, generation, and initial-object placement against the observed 63.25 MiB DRAM window before assuming the product fits.
3. **Duo interrupt facts:** confirm the CV1800B PLIC S-mode context layout against the SoC documentation before selecting an upstream driver layout.
4. **Duo page-table safety:** observe the C906 `mxstatus`/`sxstatus` memory-attribute state before trusting standard Sv39 PTEs.
5. **Minimum physical seL4 boot:** carry the existing FIT and USB-NCM deployment loop through elfloader, seL4, timer/interrupt bring-up, and `slime-root` ready evidence over serial.
6. **Verified generation:** admit one exact `riscv64-sel4-milkv-duo` generation and reject wrong-target or unsupported profiles before executable mapping.
7. **Component vertical slice:** run two isolated components through the existing bounded C7/C8 path, then observe fault supervision, restart, reclamation, and repeatable normalized transcripts.
8. **Product-target decision:** only after the vertical slice is measured decide whether Duo should host a product workload; no ROS, storage, network, or daily-driver release is implied by architecture bring-up.
