...
| [Core runtime](02-core-runtime.md) | C7 and all of C8 (C8.1–C8.15) complete; the C8 track closed 2026-08-17 with C8.14's fault-isolation envelope and C8.15's aggregate determinism gate (279 trace records across four boots, compared per worker and record kind after B68 found the flat comparison was asserting one scheduling interleaving, and per declared field after B75 found three of them were observations of the run rather than of the composition). C10.1–C10.3 closed 2026-08-23: one task-private 2 MiB window per child, an authenticated `private-memory-budget/v1` generation resource fixing every component's ceiling, and a `GlobalAlloc` over that region so the declared quota is reachable by ordinary `Vec`/`Box`/`String` — batched growth with reuse and exhaustion both asserted from the root's own records rather than the allocator's self-count | C10.4's adoption, reclamation, and leak evidence: proving a region returns in full when its holder dies, including under supervision restart, which is what makes a declared ceiling reusable across a component's lifetime rather than once per boot. Then C9 robot runtime authority |
