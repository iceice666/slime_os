...
| [Architecture portability](07-architecture-portability.md) | The AArch64 and RV64 lanes are retained; P3.D–P3.F physically qualified the Milk-V Duo. **P6.1–P6.4 closed 2026-09-02/03**: both x86-64 profiles are admitted behind their own `SLIME_X86_64_SEL4_V1` ABI, pinned QEMU q35/OVMF boots upstream seL4 and `slime-root` through one GRUB Multiboot2 EFI tree, the root's native task/IPC/fault/timer/reclamation slice runs, and the resident `init`/`console`/`spawn-service`/Slisp graph serves live COM1 input | P6.5 — build one deterministic GPT/ESP image from that same EFI tree and boot the exact raw image under QEMU/OVMF |
