...
| [Core runtime](02-core-runtime.md) | C7 and all of C8 (C8.1–C8.15) complete; the C8 track closed 2026-08-17 with C8.14's fault-isolation envelope and C8.15's aggregate determinism gate (279 trace records across four boots, compared per worker and record kind after B68 found the flat comparison was asserting one scheduling interleaving, and per declared field after B75 found three of them were observations of the run rather than of the composition). C10.1 and C10.2 closed 2026-08-23: one task-private 2 MiB window per child, grown on demand and fully reclaimed, with an authenticated `private-memory-budget/v1` generation resource fixing every component's ceiling — validated eagerly against this root's own constants, refusing aggregate over-commitment per B8, and denying any component the budget omits | C10.3's `slime-rt` `GlobalAlloc` over the private region, which is what makes the declared quota reachable by ordinary `Vec`/`Box`/`String` rather than by a probe growing pages directly. Then C10.4, then C9 robot runtime authority |
