...
| [Native I/O substrate](11-io-substrate.md) | IO0, IO1, IO3, IO4 complete 2026-08-28; IO2 complete except the root cutover. Five QEMU gates: `io_queue_check`, `io_driver_authority_check`, `io_block_check`, `io_link_check`, `io_network_check`, all registered in `sel4_gate_control_check` | IO0 fixes request/epoch/lease/queue semantics; IO1 grants bounded device/MMIO/IRQ/DMA authority with numeric reclamation on death; IO2 proves userspace virtio-blk parity but the root's virtio-blk path is still the product path; IO3 proves userspace virtio-net duplex on the same substrate; IO4 enforces exact-destination networking, with IPv6/DHCP/SLAAC/listen declared and refused. All trusted-DMA on QEMU — no containment claim |
