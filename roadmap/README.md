...
| --- | --- | --- |
| [Backlog](00-backlog.md) | B1–B69 resolved; **B70 open** (component definitions and slot/route bindings are compile-time-coupled to one crate's private manifest parser) — CP0 and CP1 landed the specification and derivation halves | B70's remaining half is CP2 (runtime binding resolution) in the [Component platform track](10-component-platform.md); B61/B63/B65 each record a deliberately deferred half a future audit should start from |
...
| [Component platform](10-component-platform.md) | CP0 and CP1 complete: `contracts/component-spec/v1` (42 records) and `contracts/system-spec/v1`, from which `valid.zti` and `sel4-channel.zti` are now generated byte-identically; CP2–CP5 not started | CP2 (runtime binding resolution) has no dependency and can start immediately; it is what removes `reference.zti`'s 44 slot pins |
