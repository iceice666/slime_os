...
**Status:** Partially delivered. The three ambient generation overrides are now closed, identity-bearing closure data, and `just system_image_scenario_check` gates that. Root roles, board instrumentation, negative build cases, and the executable-changing component scenarios are not yet modelled.
