...
**Status:** Partially delivered. The three ambient generation overrides, the executable-changing component scenarios, and the root roles with their platform-qualified instrumentation are now closed, identity-bearing closure data, and `just system_image_scenario_check` gates all three. Negative build cases and the test-run migration are not yet modelled.
