...
**Status:** CP0–CP14 complete. CP12 is complete: all 42 seL4 compositions are derived from system specs. CP13 is delivered for the closure entry point: one generic command builds any of 50 generated closures with no plane flag, variant table, or composition named in builder source; its legacy-deletion clause belongs to CP15. CP14 is delivered in full: the ambient generation overrides, the executable-changing component scenarios, the root roles and their platform-qualified instrumentation, the B40 negative build cases, and the 45 frozen test-run records are all closed, typed, identity-bearing data. CP15 is substantially delivered: 44 of 49 seL4 plane gates build by closure identity, `just system_image_closure_aggregate_check` and the two-clean-corpus-build reproducibility check are green, and the legacy plane-flag surface shrank from 40 to 7 named survivors; the SDK publication clause is the sole remaining deliverable.
...
**Status:** Substantially delivered. 44 of 49 seL4 plane gates build by closure identity — every gate whose image a closure can describe, including the product-graph and device planes CP12's closure-model fix and a `qemuKeyboard` root parameter unblocked, the two CP12 residuals (`sel4-matrix`, `sel4-c-runtime`) once those compositions derived, and the freestanding Slisp core plane. The boot-layout composer now builds all 31 layouts by closure identity with no legacy fallback. The aggregate inventory gate, the two-clean-corpus-build reproducibility check, and the aggregate's ambient-knob and dual-path source guards are green. Only the SDK publication clause — an external consumer building and booting one closure without a `slime_os` checkout — remains unstarted, and 2 gates (`sel4-boot-selection`, `sel4-root-boot`) stay on the legacy path for reasons outside this milestone's reach.
...
**Not delivered:** the SDK publication clause. `scripts/lib/component_sdk.py` exports a component-development SDK — crates, a pinned seL4 prefix, target specs, linker scripts, and a build/update tool — but no closure, generation builder, image packager, or QEMU runner; `build-system-image.py` dynamically imports repository-only `build-generation.py`, and `system_image_closure.py` hard-codes `ROOT` and requires `slime-root`, component specs, and the fifteen `RELEASE_INPUTS` paths by repository-relative location. Turning one declared closure into a verified bootable image without a `slime_os` checkout needs a second export surface with root/loader sources, the closure resolver and its direct dependencies, and a portable root-relative path convention, none of which is started.
...
**Exit condition (observed, partial):** `just sel4_gate_control_check` (45 gates, 1748 mutations), `just sel4_boot_layout_check` (31 layouts, all closure-built), `just system_image_closure_aggregate_check`, `just system_image_builder_check --exhaustive` (50 closures, two clean corpus builds, byte-identical), `just system_image_scenario_check`, `just system_test_run_check` (45 records, 44 resolving their own plane's closure), and every migrated QEMU plane gate pass. Every SDK gate that still calls the legacy builder (`component_sdk_export_check`, `component_sdk_prefix_check`, `component_sdk_release_check`, `component_sdk_compatibility_check`, `component_sdk_upgrade_check`, `component_sdk_out_of_tree_check`, `external_component_admission_check`) passes against the reduced flag surface. `just contracts_check`, `just generation_check`, `just test_host`, `just lint_all`, `just fmt_check_all`, `just ruff`, `just devlog_check`, and `just machete` all pass. The SDK publication clause is the sole remaining deliverable.
