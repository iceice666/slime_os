...
**Status:** CP0–CP11 complete. CP12 is substantially delivered — 40 of the 42 seL4 compositions are derived from system specs and the remaining 2 are recorded with closed reasons by `contracts/composition-inventory/v1` — and is the next open gate. CP13 is partially delivered: one generic command builds any of 43 generated closures, with the legacy flag family retained until CP15 migrates its last checker. CP14 is partially delivered: four of five deliverables — the ambient generation overrides, the component scenarios, the root roles, and the B40 negative cases — are closed, typed closure data. CP15 is planned. CP7's hosted-publication clause closed 2026-08-26: SDK 1.0.0 and 1.1.0 are immutable commits and signed tags on the canonical repository, both recording a source commit present on `origin/main`. `slime_os` stays authoritative; the SDK is a generated one-way mirror described by `contracts/component-sdk-release/v1`.
...
**Status:** Partially delivered. The ambient generation overrides, the executable-changing component scenarios, the root roles with their platform-qualified instrumentation, and the B40 negative build cases are now closed, typed, identity-bearing data, and `just system_image_scenario_check` gates all four. Only the test-run migration remains.
...
**Not delivered:** the fifth deliverable. QEMU disks, device topology, corruption schedules, and runtime fault controls have not moved into `system-test-run/v1`; the contract has fields for them and one record uses them, but the 36 plane checkers still own their fixtures directly, and moving them is coupled to CP15's checker migration. The `generationCmd*`, `bootSelectionFail`, and `recoveryImage` profiles and the `boot-selector` root role are declared, resolvable, and gated but not yet carried by a closure, because their host compositions build through the legacy path CP15 migrates.

**Gates:** `just system_image_scenario_check`, `just system_image_builder_check`, `just system_image_closure_check`, `just ruff`.
