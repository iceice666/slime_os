...
**Status:** Partially delivered. 41 of 49 seL4 plane gates build by closure identity — every gate whose image a closure can describe — the aggregate inventory gate is green, and the eleven plane flags migration made unreachable are deleted. The 6 remaining gates are blocked on inputs outside CP15, and the SDK publication clause is open.

**Delivered:** the aggregate inventory gate in full, and the first, second, and third deliverables in part. CP14's `boot-selector` root role is now carried by `sel4-boot-selector`, a closure over `sel4-channel`: a selector root embeds no generation, so its base composition supplies only build inputs and need not be the undrivable `sel4` graph the legacy variant named.
...
**Not delivered:** 6 gates remain on plane flags, each blocked on an input CP15 cannot supply rather than on migration work. The component-graph and device gates boot `slime-sel4-graph.elf` from the `sel4` composition, which admits an external product Slisp with no committed content identity; the matrix and c-runtime gates are CP12's two hand-authored residuals, whose blockers are recorded in `roadmap/00-backlog.md`; the boot-selection gate reads a per-arm `boot_bundle_identity` from the identity manifest, which is test-run data rather than a build key and so would need one closure per arm to become closure data; and the root-boot aggregate builds a platform image with no plane of its own. Four further gates are declared dual-path by the aggregate gate — the boot-layout composer, the demo gate, and the generation and rollback gates — each with a named reason and each still routing its closure-covered planes through the closure path.

`VARIANT_MANIFESTS`, `VARIANT_TARGET_DIRS`, `VARIANT_IMAGES`, `VARIANT_GENERATION_DELTAS`, and the identity manifest's `variant` authority survive because those gates call them; the aggregate gate's flag-reachability rule forces each to be deleted as its last caller migrates, which is how the eleven deleted flags became mandatory rather than optional. The 18 legacy-only `SLIME_*` knobs are enumerated by that gate and may only shrink. The two-clean-corpus-build check, the source guard against hard-coded checker image paths, and the SDK publication clause — an external consumer turning one declared closure into a verified bootable image without a `slime_os` checkout — are not started.
