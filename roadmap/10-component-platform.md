...
**Status:** Partially delivered. The three ambient generation overrides and the executable-changing component scenarios are now closed, identity-bearing closure data, and `just system_image_scenario_check` gates both. Root roles, board instrumentation, and negative build cases are not yet modelled.
