...
**Status:** Complete, with one correction outstanding (2026-07-26 audit). Generation format v3 with `u64` rights is built and byte-identical across two builds; retained v2 generations still decode, keep their signed release authorized (the authority hash stays 32-bit for v2 — pinned by `retained_v2_authority_manifest_is_width_stable`), and pass the stage-0 admission chain (`retained_v2_generation_passes_stage0_admission`). The original "and boots" wording was scoped to admission rather than a completed boot: no v2 artifact exists to boot, and each generation embeds the kernel it runs, so a v2 rollback executes its own v2-era kernel (backlog B6, resolved). Still open: the kernel constant is renamed to `RIGHT_BUFFER_MAP`, but the manifest right is spelled `map` in `scripts/build/build-generation.py`, so the rename did not reach the host vocabulary (backlog B7). Verified under `just generation_check`, `just contracts_check` (including the boot-contracts v2/v3 decode and admission tests), `just test`, and `just transfer_check`.
