...
**Status:** Complete. Decomposed into C7.1–C7.7 so each slice introduces one primary state surface and owns an independently reviewable QEMU check, mirroring the M5/M6 sub-slice convention. Every gate passes, including the full-graph boot checks. The 2026-07-26 audit reopened this gate on two findings, both now resolved: C7.5's boot wedge (backlog B3) and the dormant live-path shared-buffer plane (backlog B4). A built generation now carries a digest-authenticated `shared-buffer-budget/v1` resource; `bootstrap` mints a `SharedBufferFactory` and validates its generation grants; and `dango` and `spawn-service` boot with distinct non-`DENY` quotas, each proving its own through the real syscalls at startup. Residual debt is evidence quality rather than missing capability: no *test* drives the syscall layer and C7.7 still composes `u64` owner ids rather than spawned tasks (backlog B5). Evidence: `devlog/2026-07-26-c7-audit/`, `devlog/2026-07-26-b3-shared-buffer-table-stack-overflow/`, `devlog/2026-07-26-b4-live-shared-buffer-budget/`.
