...
**Status:** Complete.
**Delivered:** `components/bins` builds as native AArch64 ELF images against the `sel4` transport feature. The product generation now declares six executables (`init`, `console`, `spawn-service`, `dango`, `sysinfo`, `echo-agent`) and a resident four-instance required graph: init launches console, spawn-service, and dango, then supervises them indefinitely. Dango receives only its declared console, spawn, input, directory, and shared-buffer authority; the empty product input source reports `WouldBlock`, so the shell remains at its prompt until a real input service supplies events.
**Exit condition (observed):** `just sel4_component_graph_check` boots six target-qualified ELF payloads, observes `SLIME_GRAPH healthy ... required=4 live=4 idle=4 failed=0`, init's resident-supervision marker, dango's successful shared-buffer startup probe, prompt, and first blocked input wait. The gate stops at residence rather than requiring the required services to exit and reclaim. Generation contracts, deterministic generation construction, and the derived six-slot product boot layout pass their repository checks.
**Gates:** `just sel4_component_graph_check`, `just contracts_check`, `just generation_check`
**Evidence:** [`devlog/2026-08-04-p5-2-native-component-images/`](../devlog/2026-08-04-p5-2-native-component-images/index.md), [`devlog/2026-08-27-resident-product-graph/`](../devlog/2026-08-27-resident-product-graph/index.md)
