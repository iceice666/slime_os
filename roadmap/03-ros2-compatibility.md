...
1. R0 consumes C8's native fabric, the RP5 runtime/stream envelope, and target-qualified AArch64/RPi5 artifacts. Its completion is part of the RPi5 demo path and is observed first under AArch64 QEMU, then under physical Raspberry Pi 5 through RP7.
2. R1 broadens R0 into external topic wire compatibility after the local RPi5 demo is stable unless explicitly reprioritized. It consumes H6 or a target-specific network service plus exact destination authority.
...
**Depends on:** RP5 from [RPi5 ROS 2 demo](09-rpi5-ros2-demo.md#rp5--node-and-transport-runtime-envelope), C8 stream/fabric mechanisms used behind the gateway/runtime, a bounded stream or network service for the selected local transport, and target-qualified AArch64/RPi5 artifacts.
