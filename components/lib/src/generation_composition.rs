...
//! `GENERATION_BOOT_ACTION`, a `&str` the then-single `components/bins` crate's
//! build script copied out of one per-plane profile into `OUT_DIR` and each
//! component `include!`d — so a component's *behavior* was selected at compile
//! time by one manifest, and no component could be built outside that crate.
//! CP3 has since split the components into one crate each and hoisted the
//! build-time parser into `components/build-support`.
//!
