...
# P6.6's only evidence channel on a physical Framework, which has no serial
# port: the readiness record must be readable on the panel or the boot is
# unobservable. Selecting `linear` makes `deps/sel4/src/arch/x86/multiboot.S`
# emit the Multiboot2 framebuffer request tag, without which the bootloader
# leaves the machine in EGA text mode and the kernel exports no
# `SEL4_BOOTINFO_HEADER_X86_FRAMEBUFFER` record at all.
#
# Width and height stay at 0 — "no preference" — because the pinned q35 target
# and the Framework panel do not share a mode list, and a hardcoded geometry
# would make one of the two fail in the bootloader. Depth is pinned at 32 so
# the root's renderer has one pixel format to implement; the tag is a hint, so
# the root validates what it actually receives rather than assuming this.
set(KernelMultibootGFXMode "linear" CACHE STRING "" FORCE)
set(KernelMultibootGFXWidth 0 CACHE STRING "" FORCE)
set(KernelMultibootGFXHeight 0 CACHE STRING "" FORCE)
set(KernelMultibootGFXDepth 32 CACHE STRING "" FORCE)
